module Dynare_preprocessor_jll
...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import Dynare_preprocessor_jll on Julia versions older than 1.3!")
end
...
if isdir(override_dir)
    ...
    function find_artifact_dir()
        return artifact"Dynare_preprocessor"
    end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("Dynare_preprocessor_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "Dynare_preprocessor_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"Dynare_preprocessor", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "Dynare_preprocessor_jll", "src", "Dynare_preprocessor_jll.jl"))
        @info("Dynare_preprocessor_ll dev'ed out to /home/michel/.julia/dev/Dynare_preprocessor_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("8bf4d177-eddb-5a20-998e-55ecc2283fe8"))
platforms = [Pkg.Artifacts.unpack_platform(e, "Dynare_preprocessor", artifacts_toml) for e in artifacts["Dynare_preprocessor"]]
...
if best_platform === nothing
    @debug("Unable to load Dynare_preprocessor; unsupported platform $(triplet(platform_key_abi()))")
else
